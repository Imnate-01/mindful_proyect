...
import { Entry, EmotionKey, AIAnalysis, EmotionDetection } from '@/types/journal';
import {
  Book,
  BarChart2,
  Plus,
...
const KEYWORDS: Record<EmotionKey, RegExp[]> = {
  ansiedad: [
    /\b(preocup\w*|ansiedad\w*|presi[oó]n|miedo(s)?|nervio(s)?|estr[eé]s|abrumad\w*|angust\w*)\b/i,
  ],
  calma: [/\b(calma(da|do)?|tranquil\w*|paz|respiro|relaj\w*|sereno)\b/i],
  alegria: [
    /\b(feliz|alegr\w*|content\w*|agradecid\w*|bien|entusias\w*|genial|perfecto)\b/i,
  ],
  tristeza: [
    /\b(triste\w*|llor\w*|nostal\w*|melancol\w*|decepcion\w*|mal)\b/i,
  ],
  enojo: [/\b(enoj\w*|rabia|furioso|indignado|molesto)\b/i],
  frustracion: [
    /\b(frustr\w*|irritado|harto|cansado|no (pude|pod[ií]a)|no (sali[oó]|salen)|impotente)\b/i,
  ],
  neutra: [],
};
  ...
  sentences.forEach((sentence) => {
    ...
    (Object.keys(KEYWORDS) as EmotionKey[]).forEach((emotion) => {
      KEYWORDS[emotion].forEach((regex) => {
        if (regex.test(sent)) {
          if (!map.has(emotion))
            map.set(emotion, { count: 0, phrases: new Set() });
          const d = map.get(emotion)!;
          d.count++;
          d.phrases.add(sent.slice(0, 80));
        }
      });
    });
  });
...
/** NUEVO: useEntries ahora usa Supabase en vez de localStorage */
function _deprecated_useEntries() {
  const { user } = useUser();
  ...
  const saveEntry = async (
    ...
    const payload = {
      ...
      // ambas, por si quieres conservar emotion en inglés también
...
function getWeeklyData(entries: Entry[]) {
  ...
  return days.map((d) => ({
    day: d.label,
    avgIntensity: d.entries.length
      ? d.entries.reduce((s, e) => s + e.intensity, 0) / d.entries.length
      : 0,
    dominantEmotion: d.entries.reduce(
      (acc, e) => ((acc[e.emotion] = (acc[e.emotion] || 0) + 1), acc),
...
function AnimatedCard({
  ...
  return (
    <div
      className={`transition-all duration-700 cubic-bezier(0.175, 0.885, 0.32, 1.275) ${show
...
function NuevaEntradaView({
  ...
  const [data, setData] = useState({
    emotion: 'neutra' as EmotionKey,
    secondaryEmotions: [] as EmotionKey[],
    intensity: 5,
    notes: '',
    tags: [] as string[],
  });
  ...
  const quickDetection = useMemo(
    () => detectEmotionsByPhrase(data.notes),
    [data.notes],
  );
  ...
  const applyAIAnalysis = () => {
    ...
    setData((prev) => ({
      ...
      secondaryEmotions: aiAnalysis.emotions
        .slice(1, 3)
        .map((e) => e.emotion as EmotionKey),
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto">
      ...
      <AnimatedCard>
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500">
            Selecciona la emoción que mejor te describa
          </p>
        </div>
          ...
          {(Object.keys(THEME) as EmotionKey[]).map((e) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center p-3 rounded-2xl transition-all duration-300 border-2 ${isSelected
              ...
              >
                ...
                <span
                  className={`text-xs font-bold ${isSelected ? 'text-gray-900' : 'text-gray-500'
                    }`}
                >
                  {t.name}
                </span>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="md:col-span-2 space-y-6">
          <AnimatedCard delay={100}>
            <div
              className={`bg-white/70 backdrop-blur-xl rounded-3xl p-6 border border-white/40 shadow-xl transition-colors duration-500 ${theme.ring} ring-1 ring-opacity-20`}
            >
              <div className="flex justify-between items-center mb-3">
                <label className="text-sm font-bold text-gray-700 flex items-center gap-2">
                  <Book className="w-4 h-4 text-emerald-500" /> Tu
                  Diario
                </label>
                <span className="text-xs text-gray-400 font-medium">
                  {lastSavedAt ? 'Autoguardado' : 'Escribiendo...'}
                </span>
              </div>
              <textarea
                value={data.notes}
              ...
              <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100/50">
                ...
                <button
                  ...
                  disabled={
                    isAnalyzing || data.notes.trim().length < 20
                  }
          ...
          <AnimatedCard delay={200}>
            <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-6 border border-white/40 shadow-lg">
              <label className="block mb-4">
                ...
                <span className={`text-3xl font-black ${theme.header}`}>
                  {data.intensity}
                </span>
              ...
              <div className="h-64 flex flex-col justify-center items-center">
                <input
                  ...
                  className={`appearance-none h-4 rounded-full outline-none cursor-pointer shadow-inner ${theme.slider}`}
                  style={{
                    background: `linear-gradient(to right, ${theme.color
                      } 0%, ${theme.color} ${data.intensity * 10
                      }%, #e2e8f0 ${data.intensity * 10}%, #e2e8f0 100%)`,
          ...
          <div className="bg-gradient-to-br from-violet-50 to-fuchsia-50 rounded-3xl p-8 border-2 border-violet-100 shadow-2xl relative overflow-hidden">
            ...
            <div className="relative z-10">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <p className="text-violet-600">
                    Esto es lo que noté en tu escritura...
                  </p>
                </div>
                <button
                    ...
                    {aiAnalysis.emotions.map((e, i) => (
                      ...
                      >
                        <span className="flex items-center gap-2">
                          {THEME[e.emotion as EmotionKey].emoji}
                          <span className="font-medium">
                            {THEME[e.emotion as EmotionKey].name}
                          </span>
                        </span>
                ...
                <div className="bg-white/60 rounded-2xl p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {aiAnalysis.suggestions_quick.map((s, i) => (
                      <span
                        key={i}
                        className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-md border border-emerald-200"
                      >
                        💡 {s}
                      </span>
                    ))}
              ...
              <button
                ...
                className="w-full py-3 bg-violet-600 hover:bg-violet-700 text-white font-bold rounded-xl transition-colors shadow-lg shadow-violet-200"
              >
      ...
      <AnimatedCard delay={400}>
        <button
          onClick={handleSave}
          className={`w-full py-4 rounded-2xl bg-gradient-to-r ${theme.gradFrom} ${theme.gradTo} text-white font-bold text-lg shadow-lg hover:shadow-2xl hover:scale-[1.01] transition-all flex items-center justify-center gap-3`}
        >
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center opacity-60">
        <Book className="w-16 h-16 text-gray-300 mb-4" strokeWidth={1} />
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center opacity-60">
        ...
        <p className="text-gray-400 max-w-xs mx-auto">
          Escribe tu primera entrada para empezar a ver tus patrones
          emocionales.
        </p>
      </div>
    );
            ...
            <div className="group relative bg-white/80 backdrop-blur-md border border-white/50 rounded-2xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
                ...
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right hidden sm:block">
                      ...
                      <div
                        className={`text-xl font-black ${theme.header}`}
                      >
                        {entry.intensity}
                      </div>
                    </div>
              ...
              {isExpanded && (
                <div className="px-8 pb-6 pt-2 pl-8 border-t border-gray-100 animate-in slide-in-from-top-2">
                  <p className="text-gray-700 text-lg leading-relaxed whitespace-pre-wrap mb-6 font-serif">
                    {entry.notes}
                  </p>
...
function AnalisisView({ entries }: { entries: Entry[] }) {
  ...
  const avgMood =
    entries.reduce((acc, curr) => acc + curr.intensity, 0) /
    (totalEntries || 1);
        ...
        <div className="bg-white/70 backdrop-blur-xl rounded-3xl p-8 border border-white/50 shadow-lg">
          ...
          <div className="flex items-end justify-between h-64 gap-2 md:gap-4">
            {weeklyData.map((day, i) => {
              ...
              const emotion = dominantEntry
                ? (dominantEntry[0] as EmotionKey)
                : 'neutra';
              ...
              return (
                ...
                >
                  ...
                  <div className="mt-3 text-center">
                    <div className="text-xl mb-1 filter drop-shadow-sm transition-transform group-hover:scale-125">
                      {day.count > 0 ? theme.emoji : ''}
                    </div>
...
export default function DiarioPage() {
  ...
  const [view, setView] = useState<'nueva' | 'historial' | 'analisis'>(
    'nueva',
  );
  const { entries, saveEntry, deleteEntry, loadingEntries } = useEntries();
  ...
  const handleSave = async (entry: Omit<Entry, 'id' | 'createdAt'>) => {
    const id = await saveEntry(entry);
    if (id) {
      showToast('✨ Entrada guardada en tu diario');
      setView('historial');
    } else {
      showToast('⚠️ No se pudo guardar la entrada. Intenta de nuevo.');
    }
  };

  if (loading || !user) {
    return (
      <main className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 flex items-center justify-center">
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 pb-20">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-10 space-y-2">
          ...
          <p className="text-lg text-gray-500 font-medium max-w-lg mx-auto">
            Un espacio seguro para documentar tu viaje, entender tus
            emociones y crecer cada día.
          </p>
        </div>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-2.5 rounded-full font-bold text-sm transition-all duration-300 ${isActive
      ...
      {/* Toast Notification */}
      <div
        className={`fixed bottom-8 left-1/2 -translate-x-1/2 z-50 transition-all duration-500 ${toast.show
